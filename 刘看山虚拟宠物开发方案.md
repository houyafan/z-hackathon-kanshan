...
- `docs/product-requirements-v1.4.md`
...
- 内容养成：接收回答、文章、视频、盐选试读、点赞、收藏、评论、分享等内容行为，转化为知识能量、成长值、心情值、游历精力。
- 宠物状态：维护学识值、心情值、健康值、成长等级、游历精力、游历状态、冷却状态。
...
- 养成服务：计算知识能量、成长值、心情值、健康值、等级、游历精力。
...
- `.pet-reward-popover`：知识能量、成长值、心情值浮层。
...
| 行为 | 判定条件 | 奖励 |
| --- | --- | --- |
| 普通回答/文章 | 完读 >= 70%，停留 >= 30 秒 | 基础知识能量 x1，成长值 +5，游历精力 +1 |
| 视频 | 完播或观看 >= 60% | 基础知识能量 x1，成长值 +8，游历精力 +1 |
| 盐选试读 | 试读 >= 60 秒 | 优质知识能量 x1，成长值 +10，游历精力 +2 |
| 盐选付费/完整阅读 | 支付成功或完整阅读 | 珍稀知识能量 x2，成长值 +20，游历精力 +5 |
| 点赞/收藏/评论/分享 | 有效互动 | 心情值 +10，随机道具，游历精力 +1 |
| 每日签到 + 浏览 3 条 | 当日首次达成 | 每日礼包，游历精力 +3 |
| 垂类任务 | 任务达成 | 双倍知识能量，成长值翻倍，游历精力 +5 |

### 5.4 衰减与休眠

- 学识值按时间自然衰减，建议服务端按 `lastUpdatedAt` 懒计算，避免定时写库。
- 健康值由连续有效内容消费维持，长期低学识或低活跃进入休眠。
- 休眠后禁止游历和普通互动，仅允许唤醒任务。
- 唤醒条件：完成 3 条优质内容消费，恢复基础健康和学识。
...
1. 用户点击“出门游历”。
2. 前端检查本地状态是否满足学识值和精力门槛，不满足则展示原因。
...
- `pet_profile`：用户宠物主档案。
- `pet_resource_log`：知识能量、成长值、心情、精力变动流水。
...
- 达成内容消费阈值后只发放一次奖励。
- 奖励能正确更新学识值、成长值、心情值、游历精力。
- 满足条件可出发游历，不满足时给出明确原因。
- 游历归来内容可领取，领取后写入手账。
- 归来后冷却 10 分钟，冷却期间不可重复游历。
- 学识值/健康值过低时进入休眠，完成唤醒任务后恢复。
...
用户读内容，刘看山获得状态提升；状态足够后它出门游历；游历归来带回符合用户兴趣的优质内容；用户阅读后继续养成刘看山。
